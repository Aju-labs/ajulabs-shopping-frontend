...
function SecureInput({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
...
function PlainInput({ value, onChange, placeholder, keyboard = 'default' }: {
  value: string; onChange: (v: string) => void; placeholder?: string; keyboard?: any;
}) {
  const [focused, setFocused] = useState(false);
  return (
    <View style={[s.input, focused && s.inputFocused]}>
...
export function SegurancaScreen({ onBack }: Props) {
  ...
  useEffect(() => {
    if (!token) { setLoadingPerfil(false); return; }
    EntregadorService.buscarPerfil(token).then(p => {
  ...
  const salvarDados = async () => {
    if (!token) return;
    if (nome.trim().length < 2) { Alert.alert('Erro', 'Nome muito curto.'); return; }
    if (!email.includes('@'))   { Alert.alert('Erro', 'Email inválido.'); return; }
    setSavingPerfil(true);
    try {
      await EntregadorService.atualizarDadosPessoais(token, { nome: nome.trim(), email: email.trim(), telefone: telefone.trim() });
  ...
  const alterarSenha = async () => {
    ...
    if (novaSenha.length < 6)     { Alert.alert('Erro', 'Nova senha deve ter ao menos 6 caracteres.'); return; }
    if (novaSenha !== confirmar)  { Alert.alert('Erro', 'As senhas não coincidem.'); return; }
    setSavingSenha(true);
    try {
      ...
      {loadingPerfil ? (
        ...
        <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.section}>
            ...
            <Field label="Email">
              <PlainInput value={email} onChange={setEmail} placeholder="seu@email.com" keyboard="email-address" />
            </Field>
            <Field label="Telefone">
              <PlainInput value={telefone} onChange={setTelefone} placeholder="+55 79 9 0000-0000" keyboard="phone-pad" />
            </Field>
            <TouchableOpacity
          ...
          <View style={s.section}>
            ...
            <Field label="Senha atual">
              <SecureInput value={senhaAtual} onChange={setSenhaAtual} placeholder="Sua senha atual" />
            </Field>
            <Field label="Nova senha">
              <SecureInput value={novaSenha} onChange={setNovaSenha} placeholder="Mínimo 6 caracteres" />
            </Field>
            <Field label="Confirmar nova senha">
              <SecureInput value={confirmar} onChange={setConfirmar} placeholder="Repita a nova senha" />
            </Field>
            <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  section:     { backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1, borderColor: '#E4E7F1', padding: 16, marginBottom: 16 },
  sectionHeader:{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 },
  sectionIcon: { width: 34, height: 34, borderRadius: 9, backgroundColor: '#FEF0E3', alignItems: 'center', justifyContent: 'center' },
  ...
  saveBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 4, backgroundColor: '#F2760F', borderRadius: 12, paddingVertical: 14 },
