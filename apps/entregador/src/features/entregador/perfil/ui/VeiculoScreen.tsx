...
export function VeiculoScreen({ onBack }: Props) {
  ...
  const handleSubmit = async () => {
    if (tipo !== 'bike') {
      ...
      if (!placa.trim())  { Alert.alert('Erro', 'Informe a placa do veículo.'); return; }
      if (!cnhUri)        { Alert.alert('Erro', 'Envie a foto da sua CNH.'); return; }
      if (!docUri)        { Alert.alert('Erro', 'Envie o documento do veículo.'); return; }
    }
    if (!token) return;
    ...
    try {
      const result = await EntregadorService.solicitarTrocaVeiculo(
        token,
        { tipoTransporte: tipo, modelo: tipo === 'bike' ? 'Bicicleta' : modelo.trim(), placa: tipo === 'bike' ? 'BICICLETA' : placa.trim().toUpperCase(), cor: cor.trim() || 'Não informado', ano: parseInt(ano) || new Date().getFullYear() },
        tipo !== 'bike' ? { cnhUri: cnhUri!, docVeiculoUri: docUri! } : undefined,
      );

      if (result.status === 'aprovado') {
        Alert.alert('Veículo atualizado!', 'Seu veículo foi atualizado com sucesso.', [{ text: 'OK', onPress: async () => { await fetchData(); setMode('view'); } }]);
      } else {
        Alert.alert('Solicitação enviada!', 'Seus documentos estão em análise. Em até 24h seu veículo será atualizado.', [{ text: 'OK', onPress: async () => { await fetchData(); setMode('view'); } }]);
      }
    } catch (e: any) {
  ...
  return (
    <SafeAreaView style={s.safe}>
      <Header onBack={mode === 'view' ? onBack : () => setMode('view')} title={mode === 'view' ? 'Meu Veículo' : 'Solicitar troca'} />
          ...
          <>
            ...
            {currentVehicle ? (
              <View style={s.vehicleCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.vehicleMeta}>{currentVehicle.cor} · {currentVehicle.ano}</Text>
                </View>
              ...
              <View style={[s.vehicleCard, { alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 24 }]}>
                <VehicleIcon tipo={currentTipo} size={32} color="#9099B3" />
                <Text style={{ fontSize: 13, color: '#9099B3', textAlign: 'center' }}>Nenhum veículo cadastrado</Text>
              </View>
            )}
              ...
              <>
                ...
                <View style={s.pendingCard}>
                  ...
                  <View style={s.pendingRow}>
                    <VehicleIcon tipo={pendingRequest.tipoTransporte as Tipo} size={20} color="#9099B3" />
                  ...
                  <Text style={s.pendingInfo}>Seus documentos estão em análise. Em até 24h o veículo será atualizado.</Text>
                </View>
              </>
            )}
            ...
            >
              ...
              <Text style={s.requestBtnText}>
                {pendingRequest ? 'Troca em análise' : currentVehicle ? 'Solicitar troca de veículo' : 'Cadastrar veículo'}
              </Text>
            </TouchableOpacity>

            {pendingRequest && (
              <Text style={s.pendingNote}>Uma solicitação já está em análise. Aguarde a aprovação antes de enviar outra.</Text>
            )}
          </>
        ) : (
          <>
              ...
              {TIPOS.map(t => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => { setTipo(t.id); setModelo(''); setPlaca(''); setCnhUri(null); setDocUri(null); }}
                    activeOpacity={0.8}
                  >
                    {t.lib === 'mci'
                      ? <MaterialCommunityIcons name={t.icon as any} size={22} color={ativo ? '#F2760F' : '#9099B3'} />
                      : <Ionicons name={t.icon as any} size={22} color={ativo ? '#F2760F' : '#9099B3'} />
                    }
            ...
            {tipo !== 'bike' ? (
              <>
                ...
                <View style={s.card}>
                  <FieldRow label="Modelo" value={modelo} onChange={setModelo} placeholder="Ex: Honda CG 160" />
                  <View style={s.divider} />
                  <FieldRow label="Placa" value={placa} onChange={v => setPlaca(v.toUpperCase())} placeholder="ABC-1234" />
                  <View style={s.divider} />
                  <FieldRow label="Cor" value={cor} onChange={setCor} placeholder="Ex: Vermelho" />
                  <View style={s.divider} />
                  <FieldRow label="Ano" value={ano} onChange={setAno} placeholder={String(new Date().getFullYear())} keyboard="numeric" />
                </View>
              </>
            ) : (
              <>
            ...
            {needsDocs && (
              <>
                ...
                <View style={s.docsHint}>
                  ...
                  <Text style={s.docsHintText}>Para moto/carro é necessário enviar CNH e documento do veículo para análise.</Text>
                </View>
...
function DocUploadButton({ label, uri, onPick }: { label: string; uri: string | null; onPick: () => void }) {
  return (
    <TouchableOpacity style={s.docBtn} onPress={onPick} activeOpacity={0.85}>
      ...
      <Ionicons name={uri ? 'checkmark-circle' : 'chevron-forward'} size={18} color={uri ? '#039855' : '#9099B3'} />
    </TouchableOpacity>
  );
}
...
const sr = StyleSheet.create({
  ...
  input:        { flex: 1, fontSize: 14, color: '#000933', textAlign: 'right', paddingVertical: 4, paddingHorizontal: 8, borderRadius: 8 },
...
const s = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 11, fontWeight: '700', color: '#9099B3', textTransform: 'uppercase', letterSpacing: 0.7, marginBottom: 10, marginTop: 6 },

  vehicleCard: {
    flexDirection: 'row', alignItems: 'center', gap: 14,
    backgroundColor: '#FFFFFF', borderRadius: 16, borderWidth: 1, borderColor: '#E4E7F1', padding: 16,
  },
  vehicleIconWrap: {
    width: 52, height: 52, borderRadius: 14, backgroundColor: 'rgba(242,118,15,0.1)',
  ...
  activeBadge:   { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 99, backgroundColor: 'rgba(3,152,85,0.1)' },
  activeBadgeText: { fontSize: 11, fontWeight: '700', color: '#039855' },

  pendingCard: {
    backgroundColor: '#FFFFFF', borderRadius: 16, borderWidth: 1.5, borderColor: 'rgba(242,118,15,0.3)', padding: 16, marginTop: 6,
  },
  ...
  requestBtn: {
    ...
    marginTop: 16, padding: 15, borderRadius: 14, borderWidth: 1.5, borderColor: 'rgba(242,118,15,0.4)',
    backgroundColor: 'rgba(242,118,15,0.06)',
  },
  requestBtnText: { fontSize: 14, fontWeight: '700', color: '#F2760F' },

  tipoRow:          { flexDirection: 'row', gap: 10 },
  tipoBtn:          { flex: 1, alignItems: 'center', gap: 6, paddingVertical: 14, borderRadius: 14, borderWidth: 1.5, borderColor: '#E4E7F1', backgroundColor: '#FFFFFF' },
  ...
  card:    { backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1, borderColor: '#E4E7F1', overflow: 'hidden', marginTop: 6 },
  divider: { height: 1, backgroundColor: '#E4E7F1', marginHorizontal: 14 },

  docsHint:     { flexDirection: 'row', alignItems: 'flex-start', gap: 8, padding: 12, backgroundColor: 'rgba(32,156,239,0.08)', borderRadius: 12, marginTop: 6, marginBottom: 10 },
  ...
  docBtn: {
    ...
    borderWidth: 1, borderColor: '#E4E7F1', padding: 14, marginBottom: 8,
  },
  docThumb:       { width: 48, height: 48, borderRadius: 10 },
  docPlaceholder: { width: 48, height: 48, borderRadius: 10, backgroundColor: 'rgba(242,118,15,0.1)', alignItems: 'center', justifyContent: 'center' },
  docLabel:       { fontSize: 14, fontWeight: '600', color: '#000933' },
  docStatus:      { fontSize: 12, marginTop: 2 },

  submitBtn:     { backgroundColor: '#F2760F', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 24 },
