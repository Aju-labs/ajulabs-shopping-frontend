...
export function DadosBancariosScreen({ onBack }: Props) {
  ...
  const fetchData = async () => {
    ...
    const [g, p, s] = await Promise.all([
      ...
      fetch(`${API}entregador/saques`, { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.json()).then(d => d.saques ?? []).catch(() => []),
    ]);
    setGanhos(g);
    setPerfil(p);
    setSaques(s);
  ...
  const totalSacado = saques.filter(s => s.status !== 'falhou').reduce((a, s) => a + Number(s.valor), 0);
  ...
  const handleSalvarPix = async () => {
    if (!pixValor.trim()) { Alert.alert('Erro', 'Informe a chave Pix.'); return; }
    if (!token) return;
    setSaving(true);
    try {
      await EntregadorService.atualizarDadosBancarios(token, { tipo: 'pix', chavePix: pixValor.trim() });
        ...
        <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={s.saldoCard}>
            ...
            {emAndamento > 0 && (
              <View style={s.andamentoBadge}>
                ...
                <Text style={s.andamentoText}>{emAndamento} entrega{emAndamento !== 1 ? 's' : ''} em andamento</Text>
              </View>
            )}
            <TouchableOpacity
              style={[s.sacarBtn, (saldoDisp < 10 || sacando) && { opacity: 0.6 }]}
              onPress={handleSacar} activeOpacity={0.85} disabled={sacando}
            >
              {sacando
                ? <ActivityIndicator color="#FFFFFF" size="small" />
                : <><Ionicons name="flash" size={16} color="#FFFFFF" /><Text style={s.sacarBtnText}>Sacar via Pix</Text></>
              }
            </TouchableOpacity>
          </View>
          ...
          {saques.length === 0 ? (
            ...
            saques.map((saque) => {
              ...
              const data = new Date(saque.criadoEm).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' });
              return (
                <View key={saque.id} style={s.saqueRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.saqueValor}>{brl(Number(saque.valor))}</Text>
                    <Text style={s.saqueData}>{data} · {saque.chavePix}</Text>
                  </View>
        ...
        <TouchableOpacity style={s.modalBackdrop} activeOpacity={1} onPress={() => setEditVisible(false)}>
          <View style={s.modalSheet}>
              ...
              {PIX_TIPOS.map(t => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => { setPixTipo(t.id); setPixValor(''); }}
                    activeOpacity={0.8}
                  >
            ...
            >
              {saving
                ? <ActivityIndicator color="#FFFFFF" size="small" />
                : <><Ionicons name="flash" size={15} color="#FFFFFF" /><Text style={s.saveBtnText}>Salvar chave Pix</Text></>
              }
            </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  saldoLabel:     { fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 },
  saldoValue:     { fontSize: 40, fontWeight: '800', color: '#FFFFFF', letterSpacing: -1, marginBottom: 16 },
  andamentoBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 14, marginTop: -10 },
  andamentoText:  { fontSize: 11, color: '#F2760F', fontWeight: '600' },
  sacarBtn:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#F2760F', borderRadius: 12, paddingVertical: 14 },
  sacarBtnText:   { fontSize: 14, fontWeight: '700', color: '#FFFFFF' },

  resumoRow:   { flexDirection: 'row', backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1, borderColor: '#E4E7F1', marginBottom: 12, overflow: 'hidden' },
  ...
  pixCard:   { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1, borderColor: '#E4E7F1', padding: 14, marginBottom: 20 },
  pixIcon:   { width: 40, height: 40, borderRadius: 10, backgroundColor: 'rgba(57,255,137,0.15)', alignItems: 'center', justifyContent: 'center' },
  pixLabel:  { fontSize: 11, color: '#9099B3', fontWeight: '600', marginBottom: 2 },
  pixValue:  { fontSize: 14, fontWeight: '600', color: '#000933' },
  editBtn:   { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 99, borderWidth: 1.5, borderColor: 'rgba(242,118,15,0.3)', backgroundColor: '#FEF0E3' },
  ...
  saqueRow:    { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderRadius: 12, borderWidth: 1, borderColor: '#E4E7F1', padding: 14, marginBottom: 8 },
  ...
  modalHandle:    { width: 40, height: 4, borderRadius: 99, backgroundColor: '#E4E7F1', alignSelf: 'center', marginBottom: 20 },
  ...
  tipoBtn:         { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 99, borderWidth: 1.5, borderColor: '#E4E7F1', backgroundColor: '#F6F7FB' },
  ...
  pixInput:        { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 13, borderRadius: 12, borderWidth: 1.5, borderColor: '#E4E7F1', backgroundColor: '#F6F7FB' },
  pixInputFocused: { borderColor: '#F2760F' },
  pixInputInner:   { flex: 1, fontSize: 15, color: '#000933' },

  saveBtn:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#F2760F', borderRadius: 14, paddingVertical: 15, marginTop: 18 },
  saveBtnText:  { fontSize: 15, fontWeight: '700', color: '#FFFFFF' },
  cancelBtn:    { marginTop: 10, padding: 14, alignItems: 'center', borderRadius: 14, borderWidth: 1, borderColor: '#E4E7F1' },
